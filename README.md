...
- **🖥️ 서버 관리 시스템**: 다중 서버 유형 (ComfyUI, OpenAI Compatible) 지원
- **🖼️ 이미지 타입 필드**: 커스텀 참고 이미지 필드 설정 기능
- **📂 관리자 메뉴 개선**: 탭 방식에서 개별 사이드바 메뉴로 분리
- **📝 프롬프트 데이터 관리**: 프롬프트 저장/불러오기 및 빠른 생성 기능
...
- **#5**: 서버 관리 시스템 → 다중 서버 유형 지원 및 헬스체크 기능 추가
- **#6**: 모바일 메뉴 접근성 → 햄버거 메뉴 및 대시보드 개선
- **#8**: 내 이미지 페이지 페이지네이션 오버플로우 → 스마트 페이지네이션 적용
- **#9**: 페이지네이션 컴포넌트화 및 기능 강화 → 재사용 컴포넌트 및 직접 페이지 이동 기능 추가
- **#11**: 프롬프트 데이터 관리 → 프롬프트 저장/불러오기, 빠른 생성 기능 추가
- **#12**: 참고 이미지 기능 분류 변경 → 이미지 타입 커스텀 필드 지원
...
<p align="center">
  <sub>개발: Claude Code Assistant | 마지막 업데이트: 2026년 1월 27일</sub>
</p>