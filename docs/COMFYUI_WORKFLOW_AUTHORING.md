...
같은 플레이스홀더를 모델 로더의 파일명과 우회 조건에 동시에 쓸 수 있다.

```json
"166": { "class_type": "UpscaleModelLoader",
         "inputs": { "model_name": "{{##upscale_model##}}" } },
"167": { "class_type": "ImageUpscaleWithModel",
         "inputs": { "upscale_model": ["166", 0], "image": ["122", 0] },
         "_vcc": { "bypassUnless": { "condition": "{{##upscale_model##}}",
                                     "passthrough": { "0": "image" } } } },
"165": { "class_type": "VHS_VideoCombine", "inputs": { "images": ["167", 0], ... } }
```

"없음"(`"0"`) 이면 167 이 빠지고 165 는 122 에 직결된다. 166 은 `model_name: "0"` 인 채 남지만
출력에서 도달 불가능하므로 ComfyUI 가 검증하지 않는다 (실측 — 큐 통과 확인).
...
### CRF 를 더 낮추기 전에 — 손실은 어디서 나는가 (#871 실측)

같은 시드로 코덱만 바꿔 4번 생성해 비교한 결과 (H3 R2V 864x480, 5초):

| 출력 | 크기 | 평탄부 노이즈 σ | 생성 총시간 |
|---|---|---|---|
| H.264 crf 16 | 1.88 MB | 1.86 | — |
| H.264 crf 10 | 4.61 MB | 1.90 | 94.5 s |
| AV1 crf 16 8bit | 1.69 MB | 1.79 | 91.9 s |
| AV1 crf 16 10bit | 1.79 MB | 1.75 | 95.5 s |

- **crf 16 → 10 은 보이는 차이가 없다** (두 프레임 PSNR 41 dB). 평탄부의 자잘한 노이즈는
  crf 10 에서도 그대로 — 인코더가 아니라 **모델 출력 자체**에 들어 있다. crf 14 면 충분하다.
- **AV1 의 인코딩 비용은 이 머신에서 0 에 가깝다** (H.264 와 같은 92~95초). 파일은 더 작다.
- 화질을 실제로 올리는 지렛대는 코덱이 아니라 **인코딩 전에 후처리하는 것**이다. 별도 판으로
  업스케일하면 480p 손실 인코딩을 한 번 더 통과해 압축 노이즈까지 키운다. 워크플로 안에서
  디코드 직후 처리하면 (PNG 시퀀스 후처리와 같은 효과, 디스크 왕복 없음) 평탄부 노이즈가
  27% 줄고 디테일은 유지됐다 — §3.5 의 업스케일 우회 패턴이 그 구현이다.
