...
# (선택) .env 진단 — 누락 변수 / 빈 시크릿 점검. 구버전 .env 업그레이드에 유용
npm run env:doctor
#   누락 키 자동 추가(기존 값 보존, .env.bak 백업): node scripts/env-doctor.js --fix
#   빈 시크릿 자동 생성:                              node scripts/env-doctor.js --fix --generate-secrets
#   프로덕션 .env.production 점검:                     node scripts/env-doctor.js --prod
