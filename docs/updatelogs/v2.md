...
### 신규 기능
- feat: 파이프라인 단계의 이미지 결과 — 썸네일 + 큰 보기 (#409, #413)
  - 백엔드 PipelineRun GET 응답에 `steps.imageGenerationJobId.resultImages` 깊은 populate
  - `StepImageThumbnails` 컴포넌트 — 72×72 썸네일 + 클릭 시 `ImageViewerDialog` 큰 보기
  - PipelineRunner / PipelineHistoryPanel 양쪽 모두 적용

### 수정
- fix: 사이드바 wrapper 에 PC 배경색 적용 (#414)
  - drawer paper 만 `#2c3e50` 으로 칠해져 있고 wrapper `<Box component="nav">` 는 무색이라 컨텐츠가 짧거나 스크롤될 때 흰 영역이 어정쩡하게 보이던 문제

## v2.7.0

### 신규 기능
- feat: 문서 시스템 일반화 — builtin 태그 name 기반 통일 (#400, #405)
  - 특별 flag (`isWorldviewTag`) 폐기. 시스템이 NAME 으로 builtin 태그 인식 (`'세계관'`, `'시스템 프롬프트'`).
  - `GET /api/tags/system-prompt`, `GET /api/tags/by-name/:name` lazy create 추가.
  - 세계관 탭에 chip 필터 (세계관 / 시스템 프롬프트) — 타입 전환 시 해당 type 태그 문서만 노출, 새 문서 작성 시 자동 부여.
- feat: 파이프라인 단계별 사전 컨텍스트 / 시스템 프롬프트 문서 선택 (#401, #406)
  - Pipeline.step 에 `contextDocIds` (다중) + `systemPromptDocId` (단일).
  - pipeline-level `useWorldview` boolean 제거. 단계마다 빌더에서 \"문서\" 버튼으로 연결.
  - `prompt-generate` 가 `systemPromptDocId` 받으면 작업판 system_prompt customField 보다 우선 적용. 문서 수정 시 다음 실행부터 자동 반영.
- feat: 파이프라인 영속 실행 + 백그라운드 워커 + 히스토리 + 부분 retry (#407, #408)
  - `PipelineRun` 모델 신설 + `/api/projects/:pid/pipeline-runs` CRUD + retry (`?from=N`).
  - Bull queue `pipeline run` + worker. 단계 순차 실행 (텍스트는 chat 서비스 직접 호출, 이미지는 ImageGenerationJob 큐 위임 + 폴링).
  - 작업판 탭 제거 (데이터 유지). 파이프라인 step picker 가 전체 작업판 노출.
  - 새 \"파이프라인 히스토리\" 탭 (구 작업 히스토리 대체). 카드 리스트 + 상세 (Stepper + 단계별 결과 / 재시작 / 삭제).
  - 페이지 떠나도 백엔드 계속 실행. 실패 시 \"N단계부터 재시작\" 가능.
- feat: ComfyUI DiffusionModels 폴더 모델 목록 통합 (#410, #411)
  - `UNETLoader/unet_name` 후보 추가 조회 — wan / flux 등 unet 단독 로드 모델 노출.
  - `ServerModelCache.modelItem.source` 필드 추가 (`'checkpoints' | 'diffusion_models'`). 차후 워크플로우 노드 분기에 사용.
