...
1. `dev` 브랜치에서 `feature/*` 또는 `fix/*` 브랜치 생성
	...
	2. github issue 일 경우 `Github Issue 처리` 항목 참고.
2. 작업 완료 후 PR 생성 → `dev` 브랜치로 머지
3. PR 제목: 커밋 메시지 스타일 (예: `feat: 기능 설명 (#이슈번호)`)
4. PR 본문에 변경사항 요약 포함
...
### 상시 확인 내역
- 현재 작업과 관련이 없는 신규 작업이 요청되었을 때, 기존에 반영되지 않은 변경사항이 있었다면, 기존 작업과 신규 작업이 다른 지 판단하고, commit이 이루어지지 않았다면 commit 을 우선 진행할 것.
