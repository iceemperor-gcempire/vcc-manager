...
```
main                    # 프로덕션 브랜치 (버전 태그 생성)
  └── dev               # 개발 브랜치 (상시 유지)
       └── feature/*    # 기능 개발 브랜치
```

### 브랜치 명명 규칙
- **개발 브랜치**: `dev` (상시 유지, main에서 분기)
...
- **폰트 규칙의 범위** (#730): 본문 텍스트는 `variant` 또는 타입 스케일 값(24/18/15/14/13.5/12.5/11.5/11)만 쓴다. **아이콘 크기 지정용 `fontSize` 는 예외** — MUI 는 SVG 아이콘 크기를 `fontSize` 로 주므로 타이포와 무관하다. `rem` 문자열은 쓰지 않는다 (v2 는 px 기준)
...
- **현재 개발 브랜치**: `dev`
- **마지막 릴리스**: 고정 표기하지 않음 — `git tag --sort=-creatordate | head -1` 로 확인 (수동 표기가 계속 어긋나 #689 에서 제거)
